...
// Copyright (c) 2011-2012 GlobalTourCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("GlobalTourCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #GlobalTourCoinTEST0\r");
            Send(hSocket, "WHO #GlobalTourCoinTEST0\r");
        } else {
            // randomly join #GlobalTourCoin00-#GlobalTourCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #GlobalTourCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #GlobalTourCoin%02d\r", channel_number).c_str());
        }
