...
uint256 hashGenesisBlock("0x7db09e449c2fcb04991f9fdcd13e59bbcdcfa56a84fc9353b1b555c1ff1688d3");
...
const string strMessageMagic = "GlobalTourCoin Signed Message:\n";
...
{
    // GlobalTourCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if (nHeight == 1)
		return 70000000 * COIN;
	
	if (nHeight > 210000*64)
		return nFees;
...
static const int64 nTargetSpacing = 0.25 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
	...
	static const int64	BlocksTargetSpacing			= 0.25 * 60;
	unsigned int		TimeDaySeconds				= 60 * 60 * 24;
	int64				PastSecondsMin				= TimeDaySeconds * 0.25 * 0.1;
	int64				PastSecondsMax				= TimeDaySeconds * 0.25 * 2.8;
...
{
    // GlobalTourCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "globaltourcoin genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xd4, 0xca, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GlobalTourCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GlobalTourCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GlobalTourCoinMiner(CWallet *pwallet)
{
    printf("GlobalTourCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("GlobalTourCoin-miner");
    ...
    try { loop {
        ...
        printf("Running GlobalTourCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GlobalTourCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GlobalTourCoinMiner, pwallet));
}
